...
const useMasterData = () => {
  ...
  const allConcepts =
    queryClient.getQueryData<AllConceptsResponse>([GET_ALL_CONCEPTS_QUERY_KEY])?.data.data || [];

  const allRestaurants =
    queryClient.getQueryData<AllRestaurantResponse>([GET_ALL_RESTAURANTS_QUERY_KEY])?.data.data ||
    [];

  const allNotifications =
    queryClient.getQueryData<AllNotificationResponse>([GET_ALL_NOTIFICATIONS_KEY])?.data.data || [];
